...
const reducer = (state = initialState, action) => {
  ...
  switch (action.type) {
    ...
    case FETCH_NEWS_FAILED: {
      return {...state, isLoading: false, error: result};
    }
...
export const fetchAllNews = (page = 1) => {
  console.log('fetchAllNews');
  try {
    return dispatch => {
      dispatch({type: FETCH_NEWS_LOADING, namespace: NAMESPACE});
      let URL = `https://www.firmwarehelp.com/wp-json/wp/v2/posts?per_page=10&page=${page}`;
